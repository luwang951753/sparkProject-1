...
public class MockData {
	...
	public static void mock(SparkSession sparkSession) {
		...
		for(int i = 0; i < 10; i++) {
			...
			for(int j = 0; j < 10; j++) {
				...
				String baseActionTime =
						date + " " + String.valueOf(random.nextInt(24));
		...
		JavaRDD<Row> rowsRDD = JavaSparkContext.fromSparkContext(sparkSession.sparkContext()).parallelize(rows);
		...
		rowsRDD = JavaSparkContext.fromSparkContext(sparkSession.sparkContext()).parallelize(rows);
		...
		List<Row> list2 = df2.takeAsList(1);
		for(Row _row : list2) {
			System.out.println(_row);
		}
	}
